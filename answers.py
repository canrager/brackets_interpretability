...
# %% Cache all the activations for every data sample
...
def get_out_by_components(
    ...
    """
    out_by_components = t.empty(10)

    # find all the layer names
        ...
        out = t.concat(
            [
                ...
                einops.rearrange(
                    ...
                    "batch seq n_heads d_model -> n_heads batch seq d_model",
                ),
...
# %%
comp_out = get_out_by_components(model=model, data=data)
unb_dir = get_pre_final_ln_dir(model=model, data=data)
out_by_component_in_unbalanced_dir = einops.einsum(
    comp_out[:, :, 0, :], unb_dir, "comp data emb, emb -> comp data"
)
# Subtract the mean
...
# %%
########################### Sec 3 inverstgating models internals
...
def get_attn_probs(
    ...
    """
    hook_name = utils.get_act_name("pattern", layer=layer)
    act = get_activations(model=model, toks=data.toks, names=hook_name)[:, head, :, :]
    return act
...
def get_pre_20_dir(model, data) -> Float[Tensor, "d_model"]:
    ...
    return layer2_ln_coefs.T @ get_WOV(model, 2, 0) @ pre_final_ln_dir


tests.test_get_pre_20_dir(get_pre_20_dir, model, data_mini)
# %%
pre_layer2_outputs_seqpos1 = get_out_by_components(model=model, data=data)[:-3, :, 1, :]
...
def get_out_by_neuron(
    ...
    """
    Wout = model.W_out[layer]
    mlp1_post_name = utils.get_act_name("post", layer)
    mlp1_post_act = get_activations(model=model, toks=data.toks, names=mlp1_post_name)
    if seq is not None:
        mlp1_post_act = mlp1_post_act[:, seq, :]

    out = einops.einsum(
        mlp1_post_act, Wout, "... neuron, neuron dmodel -> ... neuron dmodel"
    )
    return out
...
def get_out_by_neuron_in_20_dir(
    ...
    return einops.einsum(
        pre_20_dir, out_by_neuron, "dmodel, batch neuron dmodel -> batch neuron"
    )
...
def get_out_by_neuron_in_20_dir_less_memory(
    ...
    f_x_W_in: Float[Tensor, "batch neurons"] = get_activations(
...
def get_q_and_k_for_given_input(
    ...
    '''
    head = 0
    token_ids = tokenizer.tokenize(parens)
    _, cache = model.run_with_cache(token_ids)
    q_activations = cache["q", layer, head].squeeze(0)
    k_activations = cache["k", layer, head].squeeze(0)
...
# %% Assert cosine similarity between tokens "(" and ")"
...
ln0_fit = get_ln_fit(model, data, model.blocks[0].ln1, seq_pos=None)[0]
layer0_ln_coefs = t.from_numpy(ln0_fit.coef_).to(device)

v_L = embedding(model, tokenizer, "(").T @ layer0_ln_coefs.T @ W_OV
